...
import jk_5.nailed.client.render.{BufferRenderer, MonospaceFontRenderer}
 ...
 * @author jk-5
 */
trait BufferedGui extends GuiScreen {
  ...
  val pasteShortcut = Array("LCONTROL", "V")
  ...
  private def isPasteShortcutPressed = {
    val want = pasteShortcut.map(name => Keyboard.getKeyIndex(name.toUpperCase)).filter(_ != Keyboard.KEY_NONE)
