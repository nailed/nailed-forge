...
object BufferRenderer {
  ...
  val textureBorder = new ResourceLocation("nailed", "textures/terminal/border.png")
  ...
  def init(tm: TextureManager) = this.synchronized(if(!textureManager.isDefined){
    ...
    RenderHelper.checkError("BufferRenderer.displayLists")
  })
  ...
  def compileBackground(bufferWidth: Int, bufferHeight: Int) =
    if (textureManager.isDefined) {
      ...
      textureManager.get.bindTexture(textureBorder)
  ...
  def compileText(scale: Double, lines: Array[Array[Char]], colors: Array[Array[Short]]) =
    if (textureManager.isDefined) {
      ...
      lines.zip(colors).zipWithIndex.foreach {
        case ((line, color), i) => MonospaceFontRenderer.drawString(0, i * MonospaceFontRenderer.fontHeight, line, color)
      }
