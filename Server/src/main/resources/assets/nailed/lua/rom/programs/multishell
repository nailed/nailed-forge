...
menuMainTextColor, menuMainBgColor = colors.textColorHighlite, colors.black
...
local function resizeWindows()
    for n=1,#tProcesses do
        ...
        if bShowMenu then
            tProcess.window.reposition( 1, 2, w, h-1 )
        else
            tProcess.window.reposition( 1, 1, w, h )
        end
    end
end
...
if map.hasMappack() then
    multishell.launch({}, "rom/programs/gamemanager")
end
